...
public class ComponentInitializer {
    private static final ComplexPredicate MAIN_FILTER = (beanModel, clazz) ->
            beanModel.getBeanType() == clazz ||
                    beanModel.getParents().contains(clazz) ||
                    beanModel.getInterfaces().contains(clazz);
    ...
    private static List<BeanModel> resolveInitChain(Collection<BeanModel> list) {
        ...
        Set<Class<?>> resolvedComponents = new HashSet<>();
    ...
    private static Object getCollection(Class<?> collectionClass, Class<?> elementType, Set<BeanModel> initializedBeans, Set<BeanModel> notInitializedBeans)
        ...
        if (notInitializedBeans.stream()
                .filter(beanModel -> MAIN_FILTER.test(beanModel, elementType))
                .findFirst()
                .orElse(null) != null) {
    ...
    private static Object getObject(Class<?> objectClass, Set<BeanModel> initializedBeans) {
        return initializedBeans.stream()
                .filter(beanModel -> MAIN_FILTER.test(beanModel, objectClass))
                .map(BeanModel::getBean)
                .findFirst()
    ...
    private interface ComplexPredicate {
        boolean test(BeanModel beanModel, Class<?> clazz);
    }

    private static class CollectionInitException extends Exception {
        private static final long serialVersionUID = 1154453704309766696L;
    }
}
